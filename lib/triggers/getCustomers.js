...
async function processTrigger(msg, cfg, snapshot = {}) {
  ...
  */
    ...
    snapshot.lastCreated = snapshot.lastCreated || (new Date(0)).getTime();

    const objects = await getCustomers(cfg.apiKey, snapshot);
    ...
    if (objects.length > 0) {
      objects.forEach((elem) => {
        ...
        */
        ...
        const transformedElement = transform(elem, cfg, customerToOih);
      ...
      snapshot.lastCreated = objects[objects.length - 1].created;
